...
if (isset($_POST['btnUpdate'])) {
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "UPDATE free_plan_image SET `image`='$upload_image' WHERE `id`='$ID'";
        $db->sql($sql);
        ...
        if ($update_result == 1) {
            $error['update_slide'] = " <section class='content-header'><span class='label label-success'>Free Plan Image updated Successfully</span></section>";
        } else {
...
$sql_query = "SELECT * FROM `free_plan_image` WHERE id = '$ID'";
$db->sql($sql_query);
$res = $db->getResult();
?>

<section class="content-header">
    <h1>Edit Free Plan Image <small><a href='free_plan_image.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Free Plan Image</a></small></h1>
